...
function hideError(){ notice?.classList.add('hidden'); notice.textContent=''; }

const fmtMoney = (v) => new Intl.NumberFormat('fr-FR', { minimumFractionDigits:2, maximumFractionDigits:2 }).format(v);

/***********************
 *  SOCKET.IO (pour les futurs dons en temps réel)
 ***********************/
...
// Persistance des pixels révélés
const REVEAL_STORAGE_KEY = 'revealed_indices_v1'; // (navigateur)
...
async function initRevealStage(){
  ...
  // Génère masque doré opaque 1000x1000
  const d = maskCtx.createImageData(GRID,GRID);
  const a = d.data;
  for(let i=0;i<TOTAL_PIXELS;i++){
    const palette = [[201,173,67],[212,175,55],[184,134,11],[230,190,95],[255,215,0]];
  ...
  // Ré-applique les pixels déjà révélés (persistants)
  loadRevealedFromStorage();
  if(revealedSet.size){
    const data = maskImageData.data;
    for(const idx of revealedSet){
      const p = idx * 4; data[p+3] = 0; // alpha 0 = transparent
    }
  }
...
// Révèle "count" NOUVEAUX pixels aléatoires (jamais déjà révélés)
function revealNewPixels(count){
  if(!maskImageData || count<=0) return;
  const data = maskImageData.data;
  let added = 0, attempts = 0, maxAttempts = count * 50;
  while(added < count && attempts < maxAttempts){
    attempts++;
    const idx = (Math.random() * TOTAL_PIXELS) | 0;
    if(revealedSet.has(idx)) continue; // déjà révélé → on skip
    revealedSet.add(idx);
    const p = idx * 4;
...
// Synchronise le nombre de pixels révélés avec le total net ($ → pixels)
...
/***********************
 *  TOTALS / PROGRESS (et binding reveal)
 ***********************/
...
/***********************
 *  CHARGEMENT INITIAL
 *  – Force min 7 219,53 si l’API ne répond pas (ou renvoie moins)
 ***********************/
...
// Mise à jour temps réel (dons futurs)
...
async function confirmAndRecord(){
  ...
  addParticles(Number(amountEl.value || 1));
  ...
  applyTotalsNet(data.totalNet, data.last); // → ajoute des pixels si besoin
  await mountPaymentElement();             // nouveau Payment Element propre
}
