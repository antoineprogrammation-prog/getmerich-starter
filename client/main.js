...
async function unlockAudio(){ if(audioUnlocked||!coinSound) return; try{ coinSound.muted=true; await coinSound.play(); await new Promise(r=>setTimeout(r,10)); coinSound.pause(); coinSound.currentTime=0; coinSound.muted=false; audioUnlocked=true; }catch{} }
...
let particles=[];
function addParticles(amount){ if(!assetsReady) return; const coins=Math.min(Math.max(3,Math.floor(amount/5)),10); const bills=amount>=20?Math.min(Math.floor(amount/20),5):0;
...
(function loop(){ animCtx.clearRect(0,0,animCanvas.width,animCanvas.height);
  for(let i=particles.length-1;i>=0;i--){ const p=particles[i]; animCtx.save(); animCtx.translate(p.x,p.y); animCtx.rotate(p.r); p.img&&animCtx.drawImage(p.img,-p.img.width/2,-p.img.height/2); animCtx.restore();
    p.x+=p.vx; p.y+=p.vy; p.r+=p.vr; if(p.img===coinsImg && p.y+(p.img.height/2)>animCanvas.height){ p.vy*=-0.5; p.y=animCanvas.height-(p.img.height/2); p.vx*=0.7; } if(p.y>animCanvas.height+120) particles.splice(i,1); }
...
function drawPhotoCover(){ const cw=photoCanvas.width,ch=photoCanvas.height; photoCtx.clearRect(0,0,cw,ch); if(!photoImg) return; const iw=photoImg.width,ih=photoImg.height; const cr=cw/ch,ir=iw/ih;
  let sx,sy,sw,sh; if(ir>cr){ sh=ih; sw=ih*cr; sx=(iw-sw)/2; sy=0; } else { sw=iw; sh=iw/cr; sx=0; sy=(ih-sh)/2; } photoCtx.drawImage(photoImg,sx,sy,sw,sh,0,0,cw,ch);
}
async function initRevealStage(){ photoCanvas.width=1080; photoCanvas.height=1616; maskCanvas.width=GRID; maskCanvas.height=GRID;
  ...
  const d=maskCtx.createImageData(GRID,GRID), a=d.data, PAL=[[201,173,67],[212,175,55],[184,134,11],[230,190,95],[255,215,0]];
  for(let i=0;i<TOTAL;i++){ const base=PAL[(Math.random()*PAL.length)|0]; const r=Math.max(0,Math.min(255,base[0]+(Math.random()*26-13)));
    const g=Math.max(0,Math.min(255,base[1]+(Math.random()*26-13))); const b=Math.max(0,Math.min(255,base[2]+(Math.random()*26-13))); const j=i*4; a[j]=r; a[j+1]=g; a[j+2]=b; a[j+3]=255; }
...
function revealTo(target){ target=Math.max(0,Math.min(TOTAL,Math.floor(target))); if(!maskImageData) return; const data=maskImageData.data;
  while(revealedCount<target){ const idx=(Math.random()*TOTAL)|0; const p=idx*4; if(data[p+3]===0) continue; data[p+3]=0; revealedCount++; if(revealedCount%10000===0) maskCtx.putImageData(maskImageData,0,0); }
...
function waitStripe(){ return new Promise((ok,ko)=>{ const t=setTimeout(()=>ko(new Error('Stripe.js not loaded')),10000);
  (function check(){ if(window.Stripe){ clearTimeout(t); ok(); } else setTimeout(check,50); })(); }); }
...
async function createPI(amount,pseudo){ const r=await fetch('/api/create-payment-intent',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({amount,pseudo})});
...
async function mountPaymentElement(){
  ...
  try{
    ...
    const amount = Math.max(1, Math.floor(Number(amountEl?.value || 1))); const pseudo = (pseudoEl?.value || 'Anonymous').slice(0,50);
    ...
    if(donateBtn) donateBtn.disabled=false; if(stripeDiag) stripeDiag.classList.add('hidden');
  }catch(e){
    ...
    try{ const r=await fetch('/api/diag/stripe',{cache:'no-store'}); if(r.ok){ const d=await r.json();
      if(stripeDiag){ stripeDiag.textContent = `Stripe diag — publishableKey: ${d.havePublishableKey?'OK':'MISSING'} (prefix: ${d.publishableKeyPrefix??'n/a'}), secretKey: ${d.haveSecretKey?'OK':'MISSING'} (mode: ${d.modeHint}).`;
        stripeDiag.classList.remove('hidden'); } } }catch{}
  }
}
async function confirmAndRecord(){
  ...
  try{ coinSound.currentTime=0; await coinSound.play(); }catch{} addParticles(Math.max(1,Math.floor(Number(amountEl?.value||1))));
  const amount=Math.max(1,Math.floor(Number(amountEl?.value||1))); const pseudo=(pseudoEl?.value||'Anonymous').slice(0,50);
