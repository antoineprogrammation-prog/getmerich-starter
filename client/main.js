...
coinSound && (coinSound.volume = 1.0);
let audioUnlocked=false;
async function unlockAudio(){
  if(audioUnlocked || !coinSound) return;
  try{
    // Try immediate play in response to a user gesture
...
function addParticles(amount){
  ...
  const coins=Math.min(Math.max(3,Math.floor(amount / 5)), 15);
  const bills=amount >= 20 ? Math.min(Math.floor(amount / 20), 8) : 0;
  for(let i=0;i<coins;i++){
    particles.push({
      img:coinsImg, x:Math.random()*animCanvas.width, y:-50,
      vx:(Math.random()-0.5)*1.6, vy:2+Math.random()*2.5,
      r:Math.random()*Math.PI*2, vr:(Math.random()-0.5)*0.25
    });
  }
  for(let i=0;i<bills;i++){
    particles.push({
      img:billsImg, x:Math.random()*animCanvas.width, y:-50,
      vx:(Math.random()-0.5)*0.9, vy:1+Math.random()*1.5,
      r:Math.random()*Math.PI*2, vr:(Math.random()-0.5)*0.08
    });
  }
}
(function loop(){
...
async function initRevealStage(){
  ...
  // mosaïque dorée opaque
  const d=maskCtx.createImageData(GRID,GRID), a=d.data;
  const PAL=[[201,173,67],[212,175,55],[184,134,11],[230,190,95],[255,215,0]];
  for(let i=0;i<TOTAL;i++){
    const base=PAL[(Math.random()*PAL.length)|0];
    const vr=(Math.random()*26-13), vg=(Math.random()*26-13), vb=(Math.random()*26-13);
...
function revealTo(target){
  target=Math.max(0,Math.min(TOTAL,Math.floor(target)));
  ...
  while(revealedCount<target){
    ...
    revealedCount++;
    if((revealedCount & 8191) === 0) maskCtx.putImageData(maskImageData,0,0); // flush périodique
  }
  maskCtx.putImageData(maskImageData,0,0);
}

/***** Jauge + binding + ANIM & SON sur variations *****/
let prevTotal = 0;
function applyTotalsNet(totalNet,last){
  ...
  // ✅ ANIMATION + SON quand le total augmente (socket update ou confirmation locale)
  const delta = Math.floor(t - prevTotal);
  if (delta > 0) {
    try { if (coinSound) { coinSound.currentTime = 0; coinSound.play().catch(()=>{}); } } catch {}
    addParticles(delta);
  }
  prevTotal = t;
}
...
async function loadInitialTotals(){
  ...
  const urls=['/api/summary','/api/donations/summary','/api/total'];
  for(const u of urls){
    try{
...
async function createPI(amount,pseudo){
...
async function confirmAndRecord(){
  ...
  // ✅ Animation & son immédiats côté donneur
  const amt = Math.max(1, Math.floor(Number(amountEl?.value || 1)));
  try { if (coinSound) { coinSound.currentTime = 0; await coinSound.play(); } } catch {}
  addParticles(amt);

  const pseudo = (pseudoEl?.value || 'Anonymous').slice(0,50);
  const r = await fetch('/api/donate', {
    method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ pseudo, amount: amt })
  });
  const data = await r.json();
  if (!r.ok || !data.success) throw new Error(data.error || 'Donation save failed');

  // Mise à jour locale (le socket poussera aussi aux autres)
  applyTotalsNet(data.totalNet, data.last);

  // Remonter un Payment Element propre pour le prochain don
